...
<HTML>
	...
	<body>
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/AdminUserServlet?method=saveUser" method="post" >
			...
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						<input type="text" name="phone" value="${ user.telephone }" id="userAction_save_do_logonName" class="bg"/>
						</td>
					<td width="18%" align="center" bgColor="#f5fafe" class="ta_01">
						锁定与否：<%-- ${ user.lock ==1} --%>
					</td>
					<td class="ta_01" bgColor="#ffffff">
					<select name="lock" >
						<option  name="lock" value="0" <c:if test="${user.lock==1 }">selected</c:if>> 未锁定</option>
						<option  name="lock" value="1" <c:if test="${user.lock==1 }">selected</c:if>> 已锁定 </option>
					</select>
					</td>
				</tr>
